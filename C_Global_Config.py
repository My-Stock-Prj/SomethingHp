...
"""
[C_Global_Config.py] - Ver 4.3 (사후 검증 및 매크로 가점 시스템 도입 버전)
...
RPT_MAJOR_POWER    = 0.07    # [3단계] 20일 누적 거래대금 중 메이저 순매수 비중
RPT_FND_DAYS       = 7       # [3단계] 20일 중 연기금 매수 발생 일수
RPT_ANT_OUT_DAYS   = 2       # [3단계] 최근 n일 누적 개인 순매수량 (0 미만이어야 함)
RPT_AVG_PRICE_DEV  = 0.05    # [3단계] 메이저 추정평단 대비 현재가 이격

# [Step 4] 응축(Concentration): 기술적 돌파 및 정배열 타점 확정
RPT_PRICE_POS      = 0.90    # [4단계] 20일 고가 대비 현재가 위치
...
ANA_STRATEGY_VER   = "V4.3_Sector_Macro" # 분석에 사용된 현재 전략 버전
...
# 🧪 [시뮬레이션 전용 파라미터 (Sim_)]
Sim_ANA_STRATEGY_VER   = "Sim_V4.3_Test" 
...
# 매크로 지표와 ADB_SUM_BASE '섹터' 칼럼 간의 매칭 정의_추가보정 [cite: 7]
MACRO_SECTOR_MAP = {
    'SOX': [
        '반도체', 'IT', '소부장', '전기전자', 
        '미디어통신'  # MST 칼럼 'KRX미디어통신' 대응
    ],
    'WTI': [
        '정유', '화학', '에너지', '조선', 
        '에너지화학', # MST 칼럼 'KRX에너지화학' 대응
        '철강',      # MST 칼럼 'KRX철강' 대응[cite: 3]
        '선박',      # MST 칼럼 'KRX선박' 대응[cite: 3]
        '건설'       # MST 칼럼 'KRX건설' 대응[cite: 3]
    ],
    'NDAQ': [
        '소프트웨어', '바이오', '인터넷', 
        '자동차'     # MST 칼럼 'KRX자동차' 대응[cite: 3]
    ]
}
